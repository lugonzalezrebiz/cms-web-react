...
function lerpHexAlpha(hex: string, fromA: number, toA: number, t: number): string {
...
function hitDiamond(px: number, py: number, cx: number, cy: number, size: number): boolean {
  return Math.abs(px - cx) + Math.abs(py - cy) <= size / Math.SQRT2 + 3;
}

function hitBar(px: number, py: number, x: number, y: number, w: number, h: number): boolean {
    ...
    for (const selPass of [false, true] as const) {
      for (const ep of points) {
        ...
        if (pass === "bars") {
          ...
          drawRoundedRect(ctx, leftPx, cy - BAR_HEIGHT / 2, widthPx, BAR_HEIGHT, BAR_RADIUS);
          ...
          if (ep.timeSec >= visibleStart && ep.timeSec <= visibleEnd) {
            drawDiamond(ctx, toSecX(ep.timeSec), cy, DIAMOND_SIZE, fillColor, outlineWidth, shadowColor, shadowBlur);
          }
          if (ep.mode === "RANGE" && ep.endSec > ep.timeSec &&
              ep.endSec >= visibleStart && ep.endSec <= visibleEnd) {
            drawDiamond(ctx, toSecX(ep.endSec), cy, DIAMOND_SIZE, fillColor, outlineWidth, shadowColor, shadowBlur);
          }
        }
      }
    }
...
export const EventRow = memo(({
  ...
  const scheduleFrame = useCallback(() => {
    ...
    s.rafId = requestAnimationFrame(() => {
      ...
      drawFrame(ctx, w, h, s.points, s.visibleStart, s.visibleEnd, s.visibleDuration, {
  ...
  const isRangeDragHandle = useCallback(
    (ep: CameraEventPoint, px: number, py: number, w: number, h: number): boolean => {
      if (ep.mode !== "RANGE") return false;
      const centerY = h / 2;
      const toSecX  = (s: number) => ((s - visibleStart) / visibleDuration) * w;
      if (ep.endSec > ep.timeSec) {
        return ep.endSec >= visibleStart && ep.endSec <= visibleEnd &&
          hitDiamond(px, py, toSecX(ep.endSec), centerY, DIAMOND_SIZE);
      }
      return ep.timeSec >= visibleStart && ep.timeSec <= visibleEnd &&
  ...
  return (
    <Box sx={{ position: "absolute", top: rowIndex * ROW_HEIGHT, left: 0, right: 0, height: ROW_HEIGHT, pointerEvents: "auto" }}>
      ...
      <div role="list" aria-label={`Event points: ${row.name}`} style={a11yStyle}>
        {points.map((ep) => (
